...
file_path = 'loanstats_15000_cleaned.csv' 
...
try:
    ...
    m_col3.metric("하이브리드 모델 R² (설명력)", f"{r2_hybrid:.4f}")
    ...
    ax.set_title("Hybrid Model Performance: Actual vs Predicted")
    ax.legend()
    st.pyplot(fig)
    
    st.caption("점들이 빨간색 점선에 가까울수록 예측이 정확한 것입니다.")

except FileNotFoundError:
    st.error("데이터 파일을 찾을 수 없습니다. 'loanstats_15000_cleaned.csv' 파일이 같은 폴더에 있는지 확인해주세요.")
